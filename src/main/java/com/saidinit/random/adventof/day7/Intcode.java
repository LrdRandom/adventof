...
public class Intcode {
	...
	private int numOfOutputCalls = 0;
	...
	public int runTheTest() {
		for (int i = 0; cursor < array.length; i++) {
			...
			if (opcode == SUM) {
				...
				if (numOfOutputCalls == 0) {
					input(array, phase);
					numOfOutputCalls++;
				} else if (numOfOutputCalls == 1) {
					System.out.println("signal called");
					input(array, signal);
				} else {
					System.out.println("more inputs");
					input(array, output);
				}
				continue;
			} else if (opcode == OUTPUT) {
				output = output(array, Integer.parseInt(inst.substring(2, 3)));
				// System.out.println("An output ocurred: " + output + " (should
				// be 0, otherwise you have an error)");
				continue;
