...
public class Day8Part2 {
	...
	// TODO: maybe change ascii for black bars and white bars, recalculate the
	// whole thing see if you are screwing up the matrixes
	...
	public void printPicture() {
		...
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 25; j++) {
				int finalValue = finalPicture[i][j];
				if (finalValue == 2) {
					System.out.print(" ");
		...
		/*
		 * int noOfZeros2 = Integer.MAX_VALUE; int maybeSolution2 = 0; for (int
		 * i = 0; i < 100; i++) { System.out.println("Picture of: " + i);
		 * Integer[][] picture = pictures.get(i); int auxZeros =
		 * countNumOfZeros(picture); System.out.println("No of Zeros: " +
		 * auxZeros); if (auxZeros <= noOfZeros2) { noOfZeros2 = auxZeros;
		 ...
		 * ----- new try ----
		 * 
		 * List<Character> splitted = new ArrayList<>(); for (char c :
		 * input.toCharArray()) { splitted.add(c); }
		 * 
		 ...
		 * 
		 * // load all pictures for (int i = 0; i < 100; i++) { Integer[][] aux
		 * = new Integer[6][25]; String onePic = input.substring(i * 150, i *
		 * 150 + 150); for (int j = 0; j < 6; j++) { for (int k = 0; k < 25;
		 * k++) { Character c = onePic.charAt(j * k); aux[j][k] =
		 * Integer.valueOf(c.toString()); } } pictures.add(aux); }
		 * 
		 * int noOfZeros = Integer.MAX_VALUE; int maybeSolution = 0; for (int i
		 * = 0; i < 100; i++) { System.out.println("Picture of: " + i);
		 * Integer[][] picture = pictures.get(i); int auxZeros =
		 * countNumOfZeros(picture); System.out.println("No of Zeros: " +
		 * auxZeros); if (auxZeros <= noOfZeros) { noOfZeros = auxZeros;
		 * maybeSolution = solution(picture);
		 * System.out.println("The solution: " + maybeSolution); }
		 * 
		 * printLayer(picture); }
		 * 
		 * System.out.println("Solution: " + maybeSolution); return
		 * maybeSolution;
		 */
	}
